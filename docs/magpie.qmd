...
## Getting MAgPIE itself running

Orientation notes for setting MAgPIE up, if you have not run it before. The
MAgPIE tutorials linked above are the authoritative source; these are the points
...
- MAgPIE has a config folder. `default.cfg` in it is very large, so rather than
  editing it directly, use a start script that reads lines from the default and
  overwrites the ones it needs.
- Start scripts live in the (non-ignored) `start` folder — for example
  `test_runs.R`.
...
Save `scenario_definitions_tutorial.csv` in the `input_template/` directory beside your run file — it is tracked in git, and ProjectFlow reads it there in place (a copy in the project's `input/` would shadow it). Then name it in the run file's `__main__` block with `p.scenario_definitions_filename = 'scenario_definitions_tutorial.csv'` and you are good to go.
