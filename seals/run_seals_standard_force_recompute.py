"""Standard SEALS run that forces recompute by taking a fresh timestamped project dir each run (under projects/tests/)."""
import os
...
def run_project(scenario_definitions_filename='standard_scenarios.csv',
                project_name='test_standard_force_recompute',
                extra_dirs=None,
                append_timestamp=False,
                tasks_to_skip=None,
                execute=True):
    """Build and execute the standard SEALS pipeline, forcing recompute.

    The __main__ run passes append_timestamp=True so each run gets a fresh
    project dir and nothing is reused — everything recomputes from scratch.
    append_timestamp=False reuses a stable project_name dir so repeated runs
    resume in place. tasks_to_skip pares the tree for variant runs;
    execute=False stops before p.execute(). Returns p.
    """
    ...
    p.extra_dirs = extra_dirs if extra_dirs is not None else ['Files', 'seals', 'projects', 'tests']
