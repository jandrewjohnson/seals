"""PRE-CONVERSION copy of run_seals_standard.py. Do not run; kept for comparison.

This is the signature-configured form the run-file spec replaced: configuration
lives in run_project's keyword defaults rather than on the ProjectFlow the caller
hands it. Diff it against run_seals_standard.py to see the conversion in one step.

The stack-wide *_old_spec.py archives were retired in favour of git history; this
one is kept deliberately because run_seals_standard.py is the reference SEALS run
and the before/after is worth having side by side.
"""
import os
